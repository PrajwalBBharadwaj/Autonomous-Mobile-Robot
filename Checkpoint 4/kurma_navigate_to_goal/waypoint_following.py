	...
	def move_robot(self):
		...
		if self.distance_error<dt:
			...
			if self.waypoint==len(self.goal)-1:
				...
				self.wait_time(10,time.time())

				#self.globalPos.x,self.globalPos.y=self.goal[self.waypoint][0],self.goal[self.waypoint][1]
				self.waypoint+=1	
				print('New Waypoint',self.waypoint)		
		else:
			move=0
			rotate=0
			if self.min_dist > self.thr1:
				print('Goto goal')
				move = 0.1*round(self.distance_error,2)
				rotate = 0.2*round(self.angle_error,2)

			else:
				if self.front_obs_dist > self.thr1: 	
					if self.min_dist_angle > 45 or self.min_dist_angle < 315:
						print('Goto goal')
						move = 0.1*round(self.distance_error,2)
						rotate = 0.2*round(self.angle_error,2)
					else:
			...
			move_cmd.linear.x=max(move,0.06)
			move_cmd.angular.z=rotate
			self._vel_publish.publish(move_cmd)
